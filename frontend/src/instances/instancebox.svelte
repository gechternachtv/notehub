...
<div class="instancebox">
    ...
    <div class="workspacecontainer viewcard-container">
        {#each workspaces.filter(e => e.instance === instance.id) as workspace}
                    ...
                    {#if workspace.img}
                    <div class="img-c">
                        <img loading="lazy" style="object-position: 0px {workspace.position}%;" src="{import.meta.env.VITE_API_URL}/api/files/{workspace.collectionId}/{workspace.id}/{workspace.img}?thumb=200x200" alt="">
                    </div>
                   {:else}
